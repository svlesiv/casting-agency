...
### create database file: pg_dump dbname > dbexport.pgsql
...
class CastingAgencyTestCase(unittest.TestCase):
    ...
    def test_update_actor_director_error(self):
        res = self.client().patch('/actors/6', json=self.update_actor_malformed, 
                                 headers={
            'Authorization': "Bearer {}".format(self.director_token)
        })
        self.assertEqual(res.status_code, 422)
    ...
    #     res = self.client().patch('/movies/1', json=self.update_movie_malformed, 
    #                              headers={
    #         'Authorization': "Bearer {}".format(self.director_token)
    #     })
    #     self.assertEqual(res.status_code, 422)
